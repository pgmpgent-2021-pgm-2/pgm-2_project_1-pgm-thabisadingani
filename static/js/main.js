...
(() => {
  ...
  updatePGMUsersRepoUI(repoPGM){
    ...
    const userRepo = repoPGM.map((user) => {
      return`
    ...
    this.$userRepository.innerHTML = userRepo;
   },
    ...
    //  console.log(followersPGM);
      const followers = followersPGM.map((user) => {
        return`
  ...
  updateGitHubUsersUI(gitName){
    ...
    updateGitUsersFollowerUI(followersGit){
      ...
      if(followersGit.length !== 0){
        ...
        this.$userFollowers.innerHTML = `<li>er zijn geen followers</li>`;
      }
    },
    calculateAge(age){ 
